...
1.  Inferring growth using the state-space model;
2.  Sampling models in STAN: A (very brief) overview
3.  Visualizing process and data uncertainty
4.  Visualizing chronology-like effects: year random effects
5.  Using inferred growth to determine tree-level biomass
6.  Comparing tree-level RW-only model to data-based biomass estimates
6.  Scaling to stand & Calculating biomass increment from biomass.
...
1.  `data{}`: inputs read in and used as "data" in the model
2.  `parameters`{}: defines the parameters that will be estimated in the model (we will draw samples from these in the model block)
3.  `transformed parameters`{}: defines the parameters that are calculated from other parameters (we will draw samples from parameters and transform them into quantities we care about in this block)
4.  `model`{}: This is where the increment data model, diameter data model and liklihood function are defined
...
## 3. Summarizing model fit--Visualizing process and data uncertainty
...
## Evaluating model convergence--how do we know we have enough iterations sampled? 
While not a complete list, we will look at 3 ways of evaluating the stan model fit itself:
1. Effective sample size
2. Rhat
3. Visualizing traceplots
...
```
## 5.  Using inferred growth to determine tree-level biomass
#### RW-only model estimated Tree Daimaeter and Growth plots (for some trees)
...
## 6. Comparing RW-only model to our data-based estimates
...
TAXA.AGB <- TR_AGB %>% group_by(taxon, COMMON_NAME, GENUS, SPECIES, cal.year, plot) %>% 
  ...
  # ungroup() %>%
  # group_by(taxon, COMMON_NAME, GENUS, SPECIES) %>%
  # 
  # arrange(cal.year) %>% 
  # 
...
ggplot(data = agb_taxon_summary)+
  ...
  #facet_grid(model~., scales='free_y') +
  theme_light(18) +
  ylab('biomass (mg/ha)')+facet_wrap(~taxon, scales = "free_y")
...
ggplot(data = abi_taxon_summary)+
  ...
  #facet_grid(model~., scales='free_y') +
  theme_light(18) +
  ylab('biomass increment (mg/ha)')+facet_wrap(~taxon, scales = "free_y")
