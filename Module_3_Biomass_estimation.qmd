...
Recruitment makes up a small amount of AGB at these plots: \<\<\<\<\<\<\< HEAD
...
```{r}
TAXA.AGB <- TAXA.AGB %>% group_by(COMMON_NAME, GENUS, SPECIES, cal.year) %>% 
  summarise(Carbon_kg_taxa = sum(AGB_Mg_ha*0.5)) %>% filter(cal.year <= 2010)

ggplot()+
  geom_line(data = TAXA.AGB, aes(x = cal.year, y = Carbon_kg_taxa, color = COMMON_NAME), size = 2)+ylab("Taxa Aboveground Carbon (kg)")+xlab("Year")
```
