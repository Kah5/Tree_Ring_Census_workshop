...
Census.AGB <- rbind(Census.LF1, Census.LF2, Census.LF3) %>% 
  mutate(AGB_Mg_ha = AGB_kg*(1/(pi*plot_radius^2)) * (1/0.0001) * (1/1000)) # convert the tree level AGB to MG/ha)
...
Census.AGB.sums <- Census.AGB %>% group_by(taxon, `Scientific Name`, site, INV.YEAR) %>% 
  summarise(`Aboveground Biomass` = sum(AGB_Mg_ha, na.rm =TRUE))
...
#| warning: false
# Recruitment
recruit.code.census <- Census.AGB %>% select(site, ID, taxon, AGB_Mg_ha, INV.YEAR) %>% 
  group_by(site, ID, taxon) %>%
  spread( INV.YEAR, AGB_Mg_ha, fill = NA) %>%
...
ggplot(data = recruit.code.census) + 
  geom_col(aes(x = variable, y = value, fill = Recruitment), stat = "identity")+facet_wrap(~site)+ylab("Aboveground Biomass (Mg/ha)")
```
...
ggplot(data = mortality.code.census) + 
  geom_col(aes(x = INV.YEAR, y = AGB_Mg_ha, fill = Mortality), stat = "identity")+facet_wrap(~site)+ylab("Aboveground Biomass (Mg/ha)")
```
...
Average.Growth.census <- mortality.code.census %>% 
  ...
  ungroup() %>%
  mutate(tree.AGBI = (AGB_Mg_ha - prevAGB)/REMPER.mort.corrected, 
...
ggplot(data = Average.Growth.census) + 
  ...
  geom_line(aes(x = INV.YEAR, y = tree.AGBI, color = taxon, group = ID))+ facet_wrap(~site)+ylab("Tree Average Aboveground Biomass Increment (Mg/ha/year)")
...
ggplot(data = site.avgs) + 
  ...
  geom_line(aes(x = INV.YEAR, y = taxon.AGBI.avg, color = taxon))+ facet_wrap(~site)+ylab("Taxon Average AGBI (Mg/ha/year)")+xlab("Inventory Year")
...
```{r}
 # we need to adjust the biomass units from kg/plot to Mg/ha
   ...
   TR.AGB <-  TR.AGB %>% mutate(TPA_factor = ifelse(dbh >= 20, outer_factor, inner_factor)) %>% mutate(AGB_Mg_ha= AGB_kg*TPA_factor)
```
...
TAXA.C <- TR.AGB %>% group_by(COMMON_NAME, GENUS, SPECIES, cal.year, plot) %>% 
  summarise(Carbon_Mg_ha_taxa = sum(AGB_Mg_ha*0.5)) #%>% 
  # ungroup()%>%
...
Census.totals <- Census.AGB.sums %>% left_join(.,taxon.conversion) %>% 
  ...
  summarise(AGB_Mg_ha_Site = mean(`Aboveground Biomass`))
