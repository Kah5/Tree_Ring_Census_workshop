...
## State-space model for estimating biomass at HF with tree ring time-series, the diameter at time of coring, and census
...
post_RW_CENSUS = post_RW_CENSUS[,,allRWs]
...
for (t in 1:N_C){
  ...
  dbh_array_RWC[t,yrinds,] = t(post_d[,inds])
...
dbh_melt$taxon = taxon_C[dbh_melt$tree]
dbh_melt$model = rep("RW + CENSUS", nrow(dbh_melt))
...
for (tree in 1:N_C){
  for (year in 1:N_years){
    ...
    dbh_mean = mean(dbh_array_RWC[tree, year, ], na.rm=TRUE)
...
agb_array_RWC[,,] = agb_array_RWC[,,] * outer_factor
